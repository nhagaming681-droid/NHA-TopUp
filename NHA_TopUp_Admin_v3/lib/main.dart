  ...
  Widget build(BuildContext context) => StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(stream: OrderService.myOrders(), builder: (context, snap) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (_, i) {
        ...
        return Card(
          child: ListTile(
            leading: CircleAvatar(child: Icon(s == 'Completed' ? Icons.check : Icons.receipt_long)),
            title: Text('${d['game']} • ${d['amount']}'),
            subtitle: Text('ID: ${d['playerId']}\n${d['paymentMethod']} • ${d['price']}\n${label(s)}'),
          ),
        );
      },
    );
  ...
  Widget build(BuildContext context) => ListView(
    ...
    children: [
      ...
      Card(child: ListTile(
        ...
        subtitle: const Text('Admin login / order management'),
